...
namespace {
// By default, we only run two tasks in a SharedMasterSession
static int32 kDefaultTasksNum = 2;
// 1 thread to run ScheduleLoop, use 2 threads to schedule enqueued tasks
// (NOTE that the nodes scheduling in tasks will use another threadpool to
// schedule)
static const int kNumThreads = 3;
static const int kPoolingActiveDelayUsecs = 10;

static bool async_run;
}  // namespace

// SharedMasterSession wraps ClientGraph in a reference counted object.
// This way, SharedMasterSession can clear up the cache mapping Run requests to
// compiled graphs while the compiled graph is still being used.
//
// TODO(zhifengc): Cleanup this class. It's becoming messy.
class SharedMasterSession::ReffedClientGraph : public core::RefCounted {
 public:
...
Status SharedMasterSession::ReffedClientGraph::RegisterPartitions(
...
void SharedMasterSession::ReffedClientGraph::TrackFeedsAndFetches(
...
Status SharedMasterSession::ReffedClientGraph::DoBuildPartitions(
    PartitionOptions popts, ClientGraph* client_graph,
    std::unordered_map<string, GraphDef>* out_partitions) {
...
Status SharedMasterSession::ReffedClientGraph::DoRegisterPartitions(
    const PartitionOptions& popts,
    std::unordered_map<string, GraphDef> graph_partitions) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    // c->req.set_session_handle(session_handle_w_tid());
...
template <class FetchListType, class ClientRequestType,
          class ClientResponseType>
Status SharedMasterSession::ReffedClientGraph::RunPartitionsHelper(
    const std::unordered_map<StringPiece, size_t, StringPieceHasher>& feeds,
    const FetchListType& fetches, const MasterEnv* env, int64 step_id,
    int64 execution_count, PerStepState* pss, CallOptions* call_opts,
    const ClientRequestType& req, ClientResponseType* resp,
...
Status SharedMasterSession::ReffedClientGraph::RunPartitions(
...
void SharedMasterSession::ReffedClientGraph::CleanupPartitionsAsync(
...
void SharedMasterSession::ReffedClientGraph::ProcessStats(
    int64 step_id, PerStepState* pss, ProfileHandler* ph,
    const RunOptions& options, RunMetadata* resp) {
...
void SharedMasterSession::ReffedClientGraph::ProcessDeviceStats(
...
// everytime.
Status SharedMasterSession::ReffedClientGraph::CheckFetches(
...
// result.
void SharedMasterSession::ReffedClientGraph::DeregisterPartitions() {
  struct Call {
...
void BuildBuildGraphOptions1(const RunStepRequestWrapper& req,
...
void BuildBuildGraphOptions1(const PartialRunSetupRequest& req,
                             BuildGraphOptions* opts) {
  CallableOptions* callable_opts = &opts->callable_options;
  CopyAndSortStrings(
...
uint64 HashBuildGraphOptions(const int32 task_id,
                             const BuildGraphOptions& opts) {
  ...
  h = Hash64Combine(static_cast<uint64>(task_id), h);

  return h;
}

string BuildGraphOptionsString1(const BuildGraphOptions& opts) {
  string buf;
  for (const string& name : opts.callable_options.feed()) {
...
void DebugConfig(const ConfigProto& config) {
  if (!config.has_graph_options()) {
    LOG(INFO) << "Config has no graph_options";
  }
  if (!config.graph_options().has_rewrite_options()) {
    LOG(INFO) << "Config has no rewrite_options";
  }
}
...
void DebugExecutionStates(
    std::vector<std::unique_ptr<GraphExecutionState>>* execution_states) {
  LOG(INFO) << "execution_states has " << execution_states->size()
            << " elements";
  for (int i = 0; i < execution_states->size(); ++i) {
    auto es = execution_states->at(i).get();
    // LOG(INFO) << "\telement " << i << " addr: " << es
    //           << ", graph addr: " << es->full_graph();
  }
}

void DebugRequest(const RunStepRequestWrapper* req) {
  if (req == nullptr) {
    LOG(ERROR) << "Request is null";
    return;
  }
  LOG(INFO) << "Request addr: " << req;
  LOG(INFO) << "Request debug string: " << req->DebugString();
  LOG(INFO) << "session_handle: " << req->session_handle();
}

}  // namespace

void DebugReadyTasks(
    const std::vector<SharedMasterSession::Task*>* ready_tasks) {
  LOG(INFO) << "Ready tasks size: " << ready_tasks->size();
  for (int i = 0; i < ready_tasks->size(); ++i) {
    LOG(INFO) << "Ready task " << i << ", addr: " << ready_tasks->at(i);
  }
}

SharedMasterSession::SharedMasterSession(
    ...
    : num_tasks_(1),
      ...
      graph_versions_(kDefaultTasksNum, 0),
      run_graphs_(5),
      partial_run_graphs_(5),
      execution_states_(1),
      num_runnings_(kDefaultTasksNum, 0),
      tasks_pool_(Env::Default(), "Tasks_Scheduler", kNumThreads) {
  UpdateLastAccessTime();
  session_opts_.emplace_back(new SessionOptions(opt));
  container_names_.emplace_back("container-0");
  CHECK(devices_) << "device_set was null!";

  Status s = ReadBoolFromEnvVar("_TF_USE_ALIGNED_SCHEDULING", false, &aligned_scheduling);
  if (!s.ok()) {
    LOG(ERROR) << "Failed to ready env _TF_USE_ALIGNED_SCHEDULING: " << s.error_message();
  }

  LOG(INFO) << "Session " << handle_ << " #local " << env->local_devices.size()
            << " #remote " << remote_devs_->size();
  LOG(INFO) << "Start master session " << handle_ << " with config: "
            << session_opts_.back()->config.ShortDebugString();

  VLOG(1) << "Session " << handle_ << " #local " << env->local_devices.size()
          << " #remote " << remote_devs_->size();
  VLOG(1) << "Start master session " << handle_ << " with config: "
          << session_opts_.back()->config.ShortDebugString();
...
SharedMasterSession::~SharedMasterSession() {
  ...
  for (auto session_options_ptr : session_opts_) delete session_options_ptr;
  for (const auto& iter : run_graphs_) iter.second->Unref();
  for (const auto& iter : partial_run_graphs_) iter.second->Unref();
  for (const auto& iter : tasks_) delete iter.second;
}

void SharedMasterSession::UpdateLastAccessTime() {
  last_access_time_usec_.store(Env::Default()->NowMicros());
}

Status SharedMasterSession::Create(GraphDef&& graph_def,
                                   const WorkerCacheFactoryOptions& options) {
  auto session_opts = session_opts_[0];
  if (session_opts->config.use_per_session_threads() ||
      session_opts->config.session_inter_op_thread_pool_size() > 0) {
    return errors::InvalidArgument(
        "Distributed session does not support session thread pool options.");
  }
  if (session_opts->config.graph_options().place_pruned_graph()) {
    ...
    session_opts->config.mutable_graph_options()->set_place_pruned_graph(false);
  }

  GraphExecutionStateOptions execution_options;
  execution_options.device_set = devices_.get();
  execution_options.session_options = session_opts_[0];
  execution_options.container_name = container_names_[0];
  {
    mutex_lock l(mu_);
    TF_RETURN_IF_ERROR(GraphExecutionState::MakeForBaseGraph(
        std::move(graph_def), execution_options, &(execution_states_[0])));
  }

  should_delete_worker_sessions_ = true;
  return CreateWorkerSessions(0, options);
}

Status SharedMasterSession::AddTask(const int32 task_id,
                                    const SessionOptions& opt,
                                    GraphDef&& graph_def,
                                    const WorkerCacheFactoryOptions& options) {
  CHECK(task_id >= session_opts_.size()) << "Task id already exists";

  LOG(INFO) << "Add task: " << task_id << " to session: " << handle();

  session_opts_.emplace_back(new SessionOptions(opt));
  container_names_.emplace_back(strings::StrCat("container-", task_id));
  auto session_opts = session_opts_.back();
  ...
  GraphExecutionStateOptions execution_options;
  execution_options.device_set = devices_.get();
  execution_options.session_options = session_opts_[task_id];
  execution_options.container_name = container_names_[task_id];

  {
    mutex_lock l(mu_);
    if (execution_states_.size() <= task_id) {
      execution_states_.emplace_back();
    }
    TF_RETURN_IF_ERROR(GraphExecutionState::MakeForBaseGraph(
        std::move(graph_def), execution_options, &execution_states_[task_id]));

  }

  {
    mutex_lock l(tasks_mu_);
    ++num_tasks_;
    ++num_remain_tasks_;
  }

  return CreateWorkerSessions(task_id, options);
}

Status SharedMasterSession::CreateWorkerSessions(
    const int32 task_id, const WorkerCacheFactoryOptions& options) {
  ...
  for (size_t i = 0; i < worker_names.size(); ++i) {
    ...
    workers[i].request.set_session_handle(
        strings::StrCat(handle_, "_", task_id));
    if (session_opts_[task_id]
            ->config.experimental()
    ...
    if (options.cluster_def) {
      *workers[i].request.mutable_server_def()->mutable_cluster() =
      ...
      workers[i].request.set_isolate_session_state(
          session_opts_[task_id]->config.isolate_session_state());
    }
    if (session_opts_[task_id]
            ->config.experimental()
...
Status SharedMasterSession::DeleteWorkerSessions() {
  WorkerCacheInterface* worker_cache = get_worker_cache();
  const std::vector<string>& worker_names = filtered_worker_list_;
  ...
  // delete all tasks' worker_session
  BlockingCounter done(worker_names.size() * num_tasks_);
  std::vector<WorkerGroup> workers(worker_names.size());
  ...
  Status status = Status::OK();
  // Create all the workers & kick off the computations.
  for (int32 j = 0; j < num_tasks_; ++j) {
    for (size_t i = 0; i < worker_names.size(); ++i) {
      workers[i].name = &worker_names[i];
      workers[i].worker = worker_cache->GetOrCreateWorker(worker_names[i]);
      workers[i].request.set_session_handle(strings::StrCat(handle_, "_", j));
      // Since the worker may have gone away, set a timeout to avoid blocking
...
Status SharedMasterSession::ListDevices(ListDevicesResponse* resp) const {
  if (worker_cache_) {
...
Status SharedMasterSession::Extend(const ExtendSessionRequest* req,
  ...
  std::unique_ptr<GraphExecutionState> extended_execution_state;
  {
    ...
    int64& graph_version_ = graph_versions_[task_id];
    ...
    CHECK(execution_states_[task_id]);
    TF_RETURN_IF_ERROR(execution_states_[task_id]->Extend(
        req->graph_def(), &extended_execution_state));

    CHECK(extended_execution_state);
    // The old execution state will be released outside the lock.
    execution_states_[task_id].swap(extended_execution_state);
    ++graph_version_;
...
WorkerCacheInterface* SharedMasterSession::get_worker_cache() const {
  if (worker_cache_) {
...
Status SharedMasterSession::StartStep(const int32 task_id,
                                      const BuildGraphOptions& opts,
                                      bool is_partial,
                                      ReffedClientGraph** out_rcg,
                                      int64* out_count) {
  // add task_id to HashFunc to avoid same tasks with the same
  // HashBuildGraphOptions(opts);
  const uint64 hash = HashBuildGraphOptions(task_id, opts);
  {
    mutex_lock l(mu_);
    ...
    if (iter == m->end()) {
      ...
      VLOG(1) << "Unseen hash " << hash << " for "
              << BuildGraphOptionsString1(opts)
              << " is_partial = " << is_partial << "\n";
      std::unique_ptr<ClientGraph> client_graph;
      TF_RETURN_IF_ERROR(
          execution_states_[task_id]->BuildGraph(opts, &client_graph));
      WorkerCacheInterface* worker_cache = get_worker_cache();
      string handle = strings::StrCat(handle_, "_", task_id);
      auto entry = new ReffedClientGraph(
          handle, opts, std::move(client_graph), *session_opts_[task_id],
...
void SharedMasterSession::ClearRunsTable(
    std::vector<ReffedClientGraph*>* to_unref, RCGMap* rcg_map) {
...
// TODO(px): multiple tasks use their own step_id
uint64 SharedMasterSession::NewStepId(int64 graph_key) {
...
Status SharedMasterSession::PartialRunSetup(const PartialRunSetupRequest* req,
                                            PartialRunSetupResponse* resp) {
  return errors::Unimplemented(
      "SharedSession PartialRunSetup has not been implemented!");
  ...
  // Prepare.
  BuildGraphOptions opts;
  BuildBuildGraphOptions1(*req, &opts);
  int64 count = 0;
  TF_RETURN_IF_ERROR(StartStep(task_id, opts, true, &rcg, &count));
  ...
  TF_RETURN_IF_ERROR(BuildAndRegisterPartitions(task_id, rcg));

  resp->set_partial_run_handle(handle);
  return Status::OK();
}

Status SharedMasterSession::Run(CallOptions* opts,
  ...
  const int32 task_id = GetTaskId(req);
  {
    ...
    if (tasks_.find(task_id) == tasks_.end()) {
      tasks_.insert(
          std::make_pair(task_id, new Task(task_id, opts, &req, resp)));
    } else {
      tasks_[task_id]->Renew(opts, &req, resp);
    }

    // bool was_empty = ready_tasks_.empty();
    int32 ready_condition = aligned_scheduling ? num_remain_tasks_ : 1;
    bool was_ready = (ready_tasks_.size() == (ready_condition - 1));
    ready_tasks_.push_back(tasks_[task_id]);
    if (was_ready) tasks_pending_.notify_all();
  }

  Task* task = tasks_[task_id];
  {
    std::unique_lock<std::mutex> ul(task->mu_);
    task->task_running_.wait(ul, [task]() { return !task->is_running_; });
  }

  return Status::OK();
}

void SharedMasterSession::RunWithCallback(CallOptions* opts,
                                          const RunStepRequestWrapper* req,
                                          MutableRunStepResponseWrapper* resp,
                                          DoneCallback done) {
  UpdateLastAccessTime();
  const int32 task_id = GetTaskId(req);
  {
    mutex_lock l(mu_);
    if (closed_) {
      done(Status(error::FAILED_PRECONDITION, "Session is closed."));
    }
    ++num_runnings_[task_id];
  ...
  if (!status.ok()) {
    LOG(ERROR) << "Run task " << task_id
               << " in session: " << req->session_handle()
               << " failed (num_running: " << num_runnings_[task_id] << ")";
  }
...
void SharedMasterSession::MarkRunCompletion(int32 task_id) {
  mutex_lock l(mu_);
  --num_runnings_[task_id];
  bool all_is_zero = true;
  for (auto&& i : num_runnings_) {
    if (i != 0) {
      all_is_zero = false;
      break;
    }
  ...
  if (all_is_zero) {
    // only when all num_running become zero, then to notify
    num_running_is_zero_.notify_all();
  }
}

Status SharedMasterSession::BuildAndRegisterPartitions(const int32 task_id,
  ...
  const bool enable_bfloat16_sendrecv =
      session_opts_[task_id]->config.graph_options().enable_bfloat16_sendrecv();
  ...
  if (session_opts_[task_id]->config.graph_options().enable_recv_scheduling()) {
...
Status SharedMasterSession::DoPartialRun(CallOptions* opts,
                                         const RunStepRequestWrapper& req,
                                         MutableRunStepResponseWrapper* resp) {
  return errors::Unimplemented(
      "DoPartialRun in SharedSession is not implemented!");

  const int32 task_id = GetTaskId(req);
  auto cleanup =
      gtl::MakeCleanup([this, task_id] { MarkRunCompletion(task_id); });
  ...
  if (!run_state->step_started) {
    ...
    const int64 build_cost_model_after =
        session_opts_[task_id]->config.graph_options().build_cost_model_after();
    const int64 build_cost_model_every =
        session_opts_[task_id]->config.graph_options().build_cost_model();
    pss.collect_costs =
  ...
  {
    mutex_lock l(mu_);
    TF_RETURN_IF_ERROR(run_state->rcg->CheckFetches(
        req, run_state, execution_states_[task_id].get()));
  }
  ...
  if (!s.ok() || is_last_partial_run) {
    ...
    rcg->CleanupPartitionsAsync(
        run_state->step_id, [this, rcg, prun_handle, task_id](const Status& s) {
          if (!s.ok()) {
            LOG(ERROR) << "Cleanup partition error: " << s;
          }
          rcg->Unref();
          MarkRunCompletion(task_id);
          Unref();
        });
    mutex_lock l(mu_);
...
Status SharedMasterSession::CreateDebuggerState(
...
void SharedMasterSession::FillPerStepState(
    const int32 task_id, SharedMasterSession::ReffedClientGraph* rcg,
    const RunOptions& run_options, uint64 step_id, int64 count,
    PerStepState* out_pss, std::unique_ptr<ProfileHandler>* out_ph) {
...
Status SharedMasterSession::PostRunCleanup(
    const int32 task_id, SharedMasterSession::ReffedClientGraph* rcg,
    uint64 step_id, const RunOptions& run_options, PerStepState* pss,
    const std::unique_ptr<ProfileHandler>& ph, const Status& run_status,
  ...
  Ref();
  rcg->Ref();
  rcg->CleanupPartitionsAsync(step_id, [this, rcg, task_id](const Status& s) {
    if (!s.ok()) {
      LOG(ERROR) << "Cleanup partition error: " << s;
    }
    rcg->Unref();
...
Status SharedMasterSession::DoRunWithLocalExecution(
    CallOptions* opts, const RunStepRequestWrapper& req,
    MutableRunStepResponseWrapper* resp) {
  ...
  BuildBuildGraphOptions1(req, session_opts_[task_id]->config, &bgopts);
  ReffedClientGraph* rcg = nullptr;
  int64 count;
  TF_RETURN_IF_ERROR(StartStep(task_id, bgopts, false, &rcg, &count));
  ...
  FillPerStepState(task_id, rcg, req.options(), step_id, count, &pss, &ph);
  ...
  return PostRunCleanup(task_id, rcg, step_id, req.options(), &pss, ph, s,
                        resp->mutable_metadata());
}

Status SharedMasterSession::MakeCallable(const MakeCallableRequest& req,
...

  {
    ...
    TF_RETURN_IF_ERROR(
        execution_states_[task_id]->BuildGraph(opts, &client_graph));
    string handle = strings::StrCat(handle_, "_", task_id);
    callable = new ReffedClientGraph(
        handle_, opts, std::move(client_graph), *session_opts_[task_id],
        stats_publisher_factory_, false /* is_partial */, get_worker_cache(),
        !should_delete_worker_sessions_);
  }

  Status s = BuildAndRegisterPartitions(task_id, callable);
  if (!s.ok()) {
    callable->Unref();
    return s;
  }
...
Status SharedMasterSession::DoRunCallable(CallOptions* opts,
                                          ReffedClientGraph* rcg,
  ...
  std::unique_ptr<ProfileHandler> ph;
  FillPerStepState(task_id, rcg, run_options, step_id, count, &pss, &ph);
  Status s = rcg->RunPartitions(env_, step_id, count, &pss, opts, req, resp,
                                &cancellation_manager_);
  cleanup.release();  // MarkRunCompletion called in PostRunCleanup().
  return PostRunCleanup(task_id, rcg, step_id, run_options, &pss, ph, s,
                        resp->mutable_metadata());
}

Status SharedMasterSession::RunCallable(CallOptions* opts,
                                        const RunCallableRequest& req,
                                        RunCallableResponse* resp) {
  UpdateLastAccessTime();
  ReffedClientGraph* callable;
  {
    ...
    callable->Ref();
    // ++num_running_;
    ++num_runnings_[GetTaskId(req)];
  }
  core::ScopedUnref unref_callable(callable);
  return DoRunCallable(opts, callable, req, resp);
}

Status SharedMasterSession::ReleaseCallable(const ReleaseCallableRequest& req,
...
void SharedMasterSession::PollTasks() {
  for (auto task : ready_tasks_) {
    DCHECK(task->call_opts);
    DCHECK(task->req);
    DCHECK(task->resp);

    CallOptions* call_opts = task->call_opts;
    const RunStepRequestWrapper* req = task->req;
    MutableRunStepResponseWrapper* resp = task->resp;

    DoneCallback done_cb = [task](const Status& ret) {
      std::lock_guard<std::mutex> l(task->mu_);
      if (!ret.ok()) {
        LOG(ERROR) << "RunWithCallback finished with bad status: " << ret;
      } else {
        task->is_running_ = false;
        task->task_running_.notify_all();
      }
    };

    tasks_pool_.Schedule(
        [=]() { RunWithCallback(call_opts, req, resp, done_cb); });
  }

  ready_tasks_.clear();
}

void SharedMasterSession::StartScheduleLoop() {
  ...
  tasks_pool_.Schedule([this]() { ScheduleLoop(); });
}

void SharedMasterSession::StopScheduleLoop() {
  if (polling_stopped_) {
    {
      mutex_lock l(tasks_mu_);
      stop_polling_ = true;
      tasks_pending_.notify_all();
    }
...
void SharedMasterSession::ScheduleLoop() {
  while (true) {
    mutex_lock l(tasks_mu_);
    if (stop_polling_) break;
    // if (ready_tasks_.empty()) {
    //   tasks_pending_.wait(l);
    // }

    int32 ready_condition = aligned_scheduling ? num_remain_tasks_ : 1;

    if (ready_tasks_.size() < ready_condition) {
      tasks_pending_.wait(l);
    }
  
    PollTasks();

    Env::Default()->SleepForMicroseconds(kPoolingActiveDelayUsecs);
  }
  polling_stopped_->Notify();
}

void SharedMasterSession::CleanupContainerResource(const int32 task_id) {
  // TODO(px): remote devices?
  for (Device* device : devices_->devices()) {
    if (device->resource_manager()) {
      // clean containter resource in ResourceMgr
      Status s = device->resource_manager()->Cleanup(container_names_[task_id]);
      if (!s.ok()) {
        LOG(ERROR) << "Cleanup device: " << device->name() << " ResourceMgr's "
                   << container_names_[task_id] << " failed, "
                   << s.error_message();
        return;
      }
    }
...
Status SharedMasterSession::Close(const int32 task_id) {
  {
    mutex_lock l(mu_);
    CleanupContainerResource(task_id);
    // when remaining tasks, omit this close
    {
      mutex_lock task_l(tasks_mu_);
      if (--num_remain_tasks_) {
        // in case schedule loop wait for more tasks
        tasks_pending_.notify_all();
        return Status::OK();
      }
    }
...
void SharedMasterSession::GarbageCollect() {
  {
    mutex_lock l(mu_);
    closed_ = true;
...
SharedMasterSession::RunState::RunState(const std::vector<string>& input_names,
...
SharedMasterSession::RunState::~RunState() {
  if (rcg) rcg->Unref();
}

bool SharedMasterSession::RunState::PendingDone() const {
...
SharedMasterSession::Task::Task(const int32 task_handle, CallOptions* call_opts,
                                const RunStepRequestWrapper* req,
                                MutableRunStepResponseWrapper* resp)
    : task_handle_(task_handle), call_opts(call_opts), req(req), resp(resp) {}

SharedMasterSession::Task::Task(Task&& other)
    : task_handle_(other.task_handle_),
      call_opts(other.call_opts),
      req(other.req),
      resp(other.resp) {}

void SharedMasterSession::Task::Renew(CallOptions* call_opts,
                                      const RunStepRequestWrapper* req,
                                      MutableRunStepResponseWrapper* resp) {
  std::lock_guard<std::mutex> l(mu_);
  this->call_opts = call_opts;
  this->req = req;
  this->resp = resp;
